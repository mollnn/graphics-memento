...
def getz(p_screenspace, p_viewspace, j, i):
    q = np.array([j, i, 0])
...
def geti(p_screenspace, p_viewspace, vals, j, i):
    q = np.array([j, i, 0])
    pp = np.array([np.array([x[0], x[1], 0]) for x in p_screenspace])
    w = [np.linalg.norm(np.cross(pp[(i+1) % 3]-q, pp[(i+2) % 3]-q), ord=1)
         for i in range(3)]
...
while True:
    ...
    clip_n, clip_f, clip_l, clip_r, clip_t, clip_b = -1, -5, -1, 1, 1, -1
    ...
    camera_pos = np.array([3*math.sin(angle), 0.5, 3*math.cos(angle)])
    camera_gaze = -camera_pos
    camera_gaze /= np.linalg.norm(camera_gaze, ord=1)
    print(camera_pos, camera_gaze)
    # camera_up = np.array([0, 1, 0])
    camera_up = [np.cross(np.cross(camera_gaze,np.array([0., 1, 0])),camera_gaze)]
    camera_up /= np.linalg.norm(camera_up, ord=1)
    camera_handle = np.cross(camera_gaze, camera_up)
    camera_handle /= np.linalg.norm(camera_handle, ord=1)
    ...
    print(transform_view_rotate)
    print(transform_view_translate)
    ...
    light_dir /= np.linalg.norm(light_dir, ord=1)
    ...
    for triangle_ws in vertices_worldspace:
        ...
        normal /= np.linalg.norm(normal, ord=1)
        ...
        for i in range(img_h):
            for j in range(img_w):
                ...
                if check([k[:2]/k[3] for k in triangle_ss], x, y):
                    ...
                    if zval < zbuf[i][j]:
                        zbuf[i][j] = zval
                        vsx = geti([k[:2]/k[3] for k in triangle_ss],
                                   triangle_vs, [i[0] for i in triangle_vs], x, y)
                        vsy = geti([k[:2]/k[3] for k in triangle_ss],
                                   triangle_vs, [i[0] for i in triangle_vs], x, y)
                        vsz = geti([k[:2]/k[3] for k in triangle_ss],
                                   triangle_vs, [i[0] for i in triangle_vs], x, y)
                        vsp = np.array([vsx, vsy, vsz])
                        watch_dir = -vsp
                        watch_dir /= np.linalg.norm(watch_dir, ord=1)
                        h = light_dir_vs + watch_dir
                        h /= np.linalg.norm(h, ord=1)
                        c = ambi + diff * \
                            max(0, np.dot(normal, light_dir_vs)) + \
                            spec * (max(0, np.dot(normal, h)) ** spow)
                        img[i][j] = c
